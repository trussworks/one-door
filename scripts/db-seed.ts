...
async function insertReview(transaction: SeedTransaction): Promise<void> {
  ...
  if (seed.requestContentRevisions.length)
    await transaction
      .insert(requestContentRevisions)
      .values(seed.requestContentRevisions);
  if (seed.clarificationRequests.length)
    await transaction
      .insert(clarificationRequests)
      .values(seed.clarificationRequests);
  if (seed.deliveryHandoffs.length)
    await transaction.insert(deliveryHandoffs).values(seed.deliveryHandoffs);
  if (seed.requestResolutions.length)
    await transaction
      .insert(requestResolutions)
      .values(seed.requestResolutions);
}
