...
export type ModelResult =
  | { purpose: "intake_interpret"; validated: IntakeResult }
  | { purpose: "asset_match"; validated: AssetResult }
  | { purpose: "risk_assess"; validated: RiskResult };

export interface ModelReferences {
  offeringIds: ReadonlySet<string>;
  catalogItemIds: ReadonlySet<string>;
  policyRuleIds: ReadonlySet<string>;
  offeringKeys: ReadonlySet<string>;
}

type ModelParseResult =
  { success: true; data: ModelResult } | { success: false; error: z.ZodError };

export function safeParseModelResult(
  purpose: ModelPurpose,
  value: unknown,
): ModelParseResult {
  switch (purpose) {
    case "intake_interpret": {
      const parsed = intakeResultSchema.safeParse(value);
      return parsed.success
        ? { success: true, data: { purpose, validated: parsed.data } }
        : parsed;
    }
    case "asset_match": {
      const parsed = assetResultSchema.safeParse(value);
      return parsed.success
        ? { success: true, data: { purpose, validated: parsed.data } }
        : parsed;
    }
    case "risk_assess": {
      const parsed = riskResultSchema.safeParse(value);
      return parsed.success
        ? { success: true, data: { purpose, validated: parsed.data } }
        : parsed;
    }
...
export function parseModelResult(
  ...
  const parsed = safeParseModelResult(purpose, value);
...
export function providerOutputSchema(
  ...
  if (references)
    bindReferenceEnums(schema, {
      offeringId: [...references.offeringIds],
      catalogItemId: [...references.catalogItemIds],
      policyRuleId: [...references.policyRuleIds],
      relatedOfferingKeys: [...references.offeringKeys],
      id: [...references.offeringIds, ...references.catalogItemIds],
    });
  return schema;
}
