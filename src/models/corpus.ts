...
export interface CorpusSnapshot extends ModelReferences {
...
async function offeringCorpus(db: Reader): Promise<CorpusSnapshot> {
  ...
  return {
    ...
    offeringIds: new Set(rows.map((row) => row.id)),
    offeringKeys: new Set(rows.map((row) => row.offeringKey)),
...
async function intakeCorpus(db: Reader): Promise<CorpusSnapshot> {
  ...
  return {
    ...
    offeringIds: offerings.offeringIds,
    offeringKeys: offerings.offeringKeys,
    catalogItemIds: new Set(catalog.map((row) => row.id)),
    policyRuleIds: new Set(),
...
export async function collectCorpus(
  ...
  if (purpose === "asset_match") {
    ...
    return {
      ...
      catalogItemIds: new Set(rows.map((row) => row.id)),
      policyRuleIds: new Set(),
  ...
  return {
    ...
    offeringKeys: new Set(),
    catalogItemIds: new Set(),
    policyRuleIds: new Set(rows.map((row) => row.id)),
    versionsById: new Map(),
    hash,
